...
export function computeRoomStats(room: Room, players: Player[]): RoomStats {
  ...
  return { count, capacity, full, totalResonance, roles };
}
...
export function rosterToText(rooms: Room[], players: Player[]): string {
  ...
  for (const tier of TIER_ORDER) {
    ...
    for (const room of tierRooms) {
      ...
      if (members.length === 0) {
        ...
        members.forEach((m, i) => {
          lines.push(
            `   ${i + 1}. ${m.name} — ${m.class} | ${m.role} | Res ${m.resonance}${m.note ? ` | ${m.note}` : ''}`,
          );
        });
      }
      lines.push('');
  ...
  if (reserved) {
    const members = roomMembers(reserved.id, players);
  ...
  if (unassigned.length > 0) {
    ...
    unassigned.forEach((m, i) => {
      lines.push(`   ${i + 1}. ${m.name} — ${m.class} | ${m.role}`);
    });
    lines.push('');
  }
